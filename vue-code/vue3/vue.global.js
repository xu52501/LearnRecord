...
`stop,prevent,self,`   +
  ...
  function createGetter(isReadonly = false, shallow = false) {
      // 使用到的属性才会做代理收集依赖
      // target = {ccb: {ccd: 1}}
      // 初始化的时候会对state做代理，获取ccb的时候会对{ccd: 1}做代理和收集依赖，省性能
      return function get(target, key, receiver) {
          // 获取内部属性 __v_isReactive
          if (key === "__v_isReactive" /* IS_REACTIVE */) {
              return !isReadonly;
          }
          else if (key === "__v_isReadonly" /* IS_READONLY */) {
              // 获取内部属性 __v_isReadonly
              return isReadonly;
          }
              ...
              // 获取内部属性 __v_raw并且receiver 是当前观察者对象，就直接返回target
              return target;
          }
  ...
  function createReactiveObject(target, isReadonly, baseHandlers, collectionHandlers) {
    //   console.log(target);
      ...
      // 数据的形式set，map，weakmap，weakset就使用collectionHandlers
    //   console.log(collectionHandlers);
    // debugger;
    console.log(target, 'targettargettarget')
    // 对state做代理，state里面的对象再get state的时候做代理，以此类推，只会在用的时候去做代理收集依赖，节约性能
      const observed = new Proxy(target, collectionTypes.has(target.constructor) ? collectionHandlers : baseHandlers);

      // proxy代理完成之后，给target挂载当前的观察者对象，是__v_readonly代表只读的观察者，__v_reactive代表是reactive的观察者
  ...
  */
  ...
  function setupStatefulComponent(instance, isSSR) {
      ...
      if (setup) {
          ...
          currentInstance = instance;
          pauseTracking();
          // shallowReadonly(instance.props)全局挂载的props做代理
